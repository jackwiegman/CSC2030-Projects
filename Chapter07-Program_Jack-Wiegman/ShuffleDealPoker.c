...
int main(void) {
    ...
    unsigned int straightFlushHand[][2] = { {0, 1}, {0, 5}, {0, 2}, {0, 3}, {0, 4} };
    printHand(straightFlushHand, suit, face);
    sortHand(straightFlushHand);
    // printHand(straightFlushHand, suit, face);
    pairs = 0;
    pairs = findPairs(straightFlushHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(straightFlushHand);
    printHandContents(pairs, straightFlushVal);

    unsigned int flushHand[][2] = { {1, 10}, {1, 8}, {1, 4}, {1, 7}, {1, 0} };
    printHand(flushHand, suit, face);
    sortHand(flushHand);
    // printHand(flushHand, suit, face);
    pairs = 0;
    pairs = findPairs(flushHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(flushHand);
    printHandContents(pairs, straightFlushVal);

    unsigned int straightHand[][2] = { {3, 10}, {3, 7}, {2, 9}, {3, 8}, {0, 11} };
    printHand(straightHand, suit, face);
    sortHand(straightHand);
    // printHand(straightHand, suit, face);
    pairs = 0;
    pairs = findPairs(straightHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(straightHand);
    printHandContents(pairs, straightFlushVal);

    unsigned int royalFlushHand[][2] = { {3, 10}, {3, 12}, {3, 9}, {3, 0}, {3, 11} };
    printHand(royalFlushHand, suit, face);
    sortHand(royalFlushHand);
    // printHand(royalFlushHand, suit, face);
    pairs = 0;
    pairs = findPairs(royalFlushHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(royalFlushHand);
    printHandContents(pairs, straightFlushVal);

    unsigned int royalStraightHand[][2] = { {3, 10}, {2, 12}, {3, 9}, {3, 0}, {3, 11} };
    printHand(royalStraightHand, suit, face);
    sortHand(royalStraightHand);
    // printHand(royalStraightHand, suit, face);
    pairs = 0;
    pairs = findPairs(royalStraightHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(royalStraightHand);
    printHandContents(pairs, straightFlushVal);

    unsigned int twoPairHand[][2] = { {3, 10}, {3, 11}, {2, 10}, {3, 0}, {1, 0} };
    printHand(twoPairHand, suit, face);
    sortHand(twoPairHand);
    // printHand(twoPairHand, suit, face);
    pairs = 0;
    pairs = findPairs(twoPairHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(twoPairHand);
    printHandContents(pairs, straightFlushVal);

    unsigned int onePairHand[][2] = { {3, 10}, {3, 11}, {2, 10}, {3, 3}, {1, 0} };
    printHand(onePairHand, suit, face);
    sortHand(onePairHand);
    // printHand(onePairHand, suit, face);
    pairs = 0;
    pairs = findPairs(onePairHand);
    straightFlushVal = 0;
    straightFlushVal = getStraightFlushValue(onePairHand);
    printHandContents(pairs, straightFlushVal);
...
unsigned int getStraightFlushValue(unsigned int hand[][2]) {
    ...
    unsigned int handSuitInt = sortedHand[0][suitIndex]; /* Value of suit to compare for a flush */
    /* variables to keep state */
    _Bool handIsStraight = true;
    _Bool handIsFlush = true;
    _Bool handIsRoyalStraight = false;
    ...
    for (int i = 1; i < HANDSIZE; i++) {
        ...
        if (sortedHand[i][suitIndex] != handSuitInt) {
            handIsFlush = false;
            // printf("%d:%d ", sortedHand[i][suitIndex], handSuitInt);
        }
    }
    ...
    if (handIsRoyalStraight) {handVal += royalVal;}
